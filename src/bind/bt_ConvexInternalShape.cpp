 ...
 * src/vendor/bullet/src/BulletCollision/CollisionShapes/btConvexInternalShape.h:50
 */
static int btConvexInternalShape__btConvexInternalShape(lua_State *L) {
  try {
    DubUserdata *userdata = ((DubUserdata*)dub_checksdata_d(L, 1, "bt.ConvexInternalShape"));
    if (userdata->gc) {
    ...
    lua_pushfstring(L, "~btConvexInternalShape: %s", e.what());
  } catch (...) {
    lua_pushfstring(L, "~btConvexInternalShape: Unknown exception");
  }
  return dub_error(L);
}
...
static int btConvexInternalShape_localGetSupportingVertex(lua_State *L) {
  try {
    btConvexInternalShape *self = *((btConvexInternalShape **)dub_checksdata(L, 1, "bt.ConvexInternalShape"));
...
extern "C" int luaopen_bt_ConvexInternalShape(lua_State *L)
{
  ...
  luaL_register(L, NULL, btConvexInternalShape_member_methods);
  // save meta-table in bt
  dub_register(L, "bt", "ConvexInternalShape", "ConvexInternalShape");
  // <mt>
  lua_pop(L, 1);
  return 0;
}
