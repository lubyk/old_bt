...
static int btRigidBodyConstructionInfo__btRigidBodyConstructionInfo(lua_State *L) {
  try {
    DubUserdata *userdata = ((DubUserdata*)dub_checksdata_d(L, 1, "bt.RigidBody.RigidBodyConstructionInfo"));
    if (userdata->gc) {
    ...
    lua_pushfstring(L, "~btRigidBodyConstructionInfo: %s", e.what());
  } catch (...) {
    lua_pushfstring(L, "~btRigidBodyConstructionInfo: Unknown exception");
  }
  return dub_error(L);
}
...
static int btRigidBodyConstructionInfo__set_(lua_State *L) {
  ...
  btRigidBody::btRigidBodyConstructionInfo *self = *((btRigidBody::btRigidBodyConstructionInfo **)dub_checksdata_n(L, 1, "bt.RigidBody.RigidBodyConstructionInfo"));
  const char *key = luaL_checkstring(L, 2);
  int key_h = dub_hash(key, 81);
  switch(key_h) {
    case 42: {
    ...
    case 15: {
      ...
      dub_protect(L, 1, 3, "m_motionState");
      self->m_motionState = *((btMotionState **)dub_checksdata_n(L, 3, "bt.MotionState"));
      return 0;
    }
    case 46: {
      if (DUB_ASSERT_KEY(key, "m_startWorldTransform")) break;
      self->m_startWorldTransform = **((btTransform **)dub_checksdata_n(L, 3, "bt.Transform"));
      return 0;
    }
    case 5: {
      if (DUB_ASSERT_KEY(key, "m_collisionShape")) break;
      dub_protect(L, 1, 3, "m_collisionShape");
      self->m_collisionShape = *((btCollisionShape **)dub_checksdata_n(L, 3, "bt.CollisionShape"));
      return 0;
    }
    case 60: {
      if (DUB_ASSERT_KEY(key, "m_localInertia")) break;
      self->m_localInertia = **((btVector3 **)dub_checksdata_n(L, 3, "bt.Vector3"));
      return 0;
    }
    case 74: {
...
static int btRigidBodyConstructionInfo__get_(lua_State *L) {
  ...
  btRigidBody::btRigidBodyConstructionInfo *self = *((btRigidBody::btRigidBodyConstructionInfo **)dub_checksdata_n(L, 1, "bt.RigidBody.RigidBodyConstructionInfo", true));
  const char *key = luaL_checkstring(L, 2);
  // <self> "key" <mt>
  // rawget(mt, key)
  lua_pushvalue(L, 2);
  ...
  switch(key_h) {
    ...
    case 46: {
      if (DUB_ASSERT_KEY(key, "m_startWorldTransform")) break;
      dub_pushudata(L, &self->m_startWorldTransform, "bt.Transform", false);
      return 1;
    }
    case 5: {
      if (DUB_ASSERT_KEY(key, "m_collisionShape")) break;
      ...
      if (!retval__) return 0;
      dub_pushudata(L, retval__, "bt.CollisionShape", false);
      return 1;
    }
    case 60: {
      if (DUB_ASSERT_KEY(key, "m_localInertia")) break;
      dub_pushudata(L, &self->m_localInertia, "bt.Vector3", false);
      return 1;
    }
    case 74: {
      if (DUB_ASSERT_KEY(key, "m_linearDamping")) break;
