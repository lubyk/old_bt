...
static int LkDebugDrawer__LkDebugDrawer(lua_State *L) {
  try {
    DubUserdata *userdata = ((DubUserdata*)dub_checksdata_d(L, 1, "bt.LkDebugDrawer"));
    if (userdata->gc) {
    ...
    lua_pushfstring(L, "~LkDebugDrawer: %s", e.what());
  } catch (...) {
    lua_pushfstring(L, "~LkDebugDrawer: Unknown exception");
  }
  return dub_error(L);
}
...
static int LkDebugDrawer__cast_(lua_State *L) {
  ...
  LkDebugDrawer *self = *((LkDebugDrawer **)dub_checksdata_n(L, 1, "bt.LkDebugDrawer"));
 ...
 * include/LkDebugDrawer.h:47
 */
static int LkDebugDrawer_LkDebugDrawer(lua_State *L) {
  try {
    int top__ = lua_gettop(L);
    if (top__ >= 1) {
      int debug_mode = dub_checkint(L, 1);
      LkDebugDrawer *retval__ = new LkDebugDrawer(debug_mode);
      retval__->pushobject(L, retval__, "bt.LkDebugDrawer", true);
      return 1;
    } else {
 ...
 * include/LkDebugDrawer.h:144
 */
static int LkDebugDrawer_setDebugMode(lua_State *L) {
  try {
    LkDebugDrawer *self = *((LkDebugDrawer **)dub_checksdata(L, 1, "bt.LkDebugDrawer"));
...
extern "C" int luaopen_bt_LkDebugDrawer(lua_State *L)
{
  ...
  luaL_register(L, NULL, LkDebugDrawer_member_methods);
  // save meta-table in bt
  dub_register(L, "bt", "LkDebugDrawer_core", "LkDebugDrawer");
  // <mt>
  lua_pop(L, 1);
  return 0;
}
