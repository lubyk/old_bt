...
static int btDbvtBroadphase__set_(lua_State *L) {
  ...
  btDbvtBroadphase *self = *((btDbvtBroadphase **)dub_checksdata_n(L, 1, "bt.DbvtBroadphase"));
  const char *key = luaL_checkstring(L, 2);
  int key_h = dub_hash(key, 82);
  switch(key_h) {
    case 44: {
    ...
    case 71: {
      ...
      self->m_updates_call = **((unsigned **)dub_checksdata_n(L, 3, "unsigned"));
      return 0;
    }
    case 57: {
      if (DUB_ASSERT_KEY(key, "m_updates_done")) break;
      self->m_updates_done = **((unsigned **)dub_checksdata_n(L, 3, "unsigned"));
      return 0;
    }
    case 40: {
...
static int btDbvtBroadphase__get_(lua_State *L) {
  ...
  btDbvtBroadphase *self = *((btDbvtBroadphase **)dub_checksdata_n(L, 1, "bt.DbvtBroadphase", true));
  const char *key = luaL_checkstring(L, 2);
  // <self> "key" <mt>
  // rawget(mt, key)
  lua_pushvalue(L, 2);
  ...
  switch(key_h) {
    ...
    case 71: {
      if (DUB_ASSERT_KEY(key, "m_updates_call")) break;
      dub_pushudata(L, &self->m_updates_call, "unsigned", false);
      return 1;
    }
    case 57: {
      if (DUB_ASSERT_KEY(key, "m_updates_done")) break;
      dub_pushudata(L, &self->m_updates_done, "unsigned", false);
      return 1;
    }
    case 40: {
      if (DUB_ASSERT_KEY(key, "m_updates_ratio")) break;
...
static int btDbvtBroadphase_btDbvtBroadphase(lua_State *L) {
  try {
    ...
    lua_pushfstring(L, "btDbvtBroadphase: %s", e.what());
  } catch (...) {
    lua_pushfstring(L, "btDbvtBroadphase: Unknown exception");
  }
  return dub_error(L);
}
...
static int btDbvtBroadphase__btDbvtBroadphase(lua_State *L) {
  try {
    DubUserdata *userdata = ((DubUserdata*)dub_checksdata_d(L, 1, "bt.DbvtBroadphase"));
    if (userdata->gc) {
 ...
 * src/vendor/bullet/src/BulletCollision/BroadphaseCollision/btDbvtBroadphase.h:127
 */
static int btDbvtBroadphase_setVelocityPrediction(lua_State *L) {
  try {
    btDbvtBroadphase *self = *((btDbvtBroadphase **)dub_checksdata(L, 1, "bt.DbvtBroadphase"));
    btScalar prediction = dub_checknumber(L, 2);
 ...
 * src/vendor/bullet/src/BulletCollision/BroadphaseCollision/btDbvtBroadphase.h:131
 */
...
static int btDbvtBroadphase_benchmark(lua_State *L) {
  try {
    btBroadphaseInterface *p1 = *((btBroadphaseInterface **)dub_checksdata(L, 1, "bt.BroadphaseInterface"));
...
// --=============================================== CONSTANTS
static const struct dub_const_Reg btDbvtBroadphase_const[] = {
...
extern "C" int luaopen_bt_DbvtBroadphase(lua_State *L)
{
  ...
  dub_register_const(L, btDbvtBroadphase_const);

  // register member methods
  luaL_register(L, NULL, btDbvtBroadphase_member_methods);
  // save meta-table in bt
  dub_register(L, "bt", "DbvtBroadphase", "DbvtBroadphase");
  // <mt>
  lua_pop(L, 1);
  return 0;
}
