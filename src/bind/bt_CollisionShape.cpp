...
static int btCollisionShape__btCollisionShape(lua_State *L) {
  try {
    DubUserdata *userdata = ((DubUserdata*)dub_checksdata_d(L, 1, "bt.CollisionShape"));
    if (userdata->gc) {
...
static int btCollisionShape___tostring(lua_State *L) {
  btCollisionShape *self = *((btCollisionShape **)dub_checksdata_n(L, 1, "bt.CollisionShape"));
...
extern "C" int luaopen_bt_CollisionShape(lua_State *L)
{
  ...
  luaL_register(L, NULL, btCollisionShape_member_methods);
  // save meta-table in bt
  dub_register(L, "bt", "CollisionShape", "CollisionShape");
  // <mt>
  lua_pop(L, 1);
  return 0;
}
