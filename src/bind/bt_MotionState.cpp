 ...
 * src/vendor/bullet/src/LinearMath/btMotionState.h:28
 */
static int btMotionState__btMotionState(lua_State *L) {
  try {
    DubUserdata *userdata = ((DubUserdata*)dub_checksdata_d(L, 1, "bt.MotionState"));
    if (userdata->gc) {
    ...
    lua_pushfstring(L, "~btMotionState: %s", e.what());
  } catch (...) {
    lua_pushfstring(L, "~btMotionState: Unknown exception");
  }
  return dub_error(L);
}
...
static int btMotionState_getWorldTransform(lua_State *L) {
  try {
    btMotionState *self = *((btMotionState **)dub_checksdata(L, 1, "bt.MotionState"));
...
static int btMotionState___tostring(lua_State *L) {
  btMotionState *self = *((btMotionState **)dub_checksdata_n(L, 1, "bt.MotionState"));
...
extern "C" int luaopen_bt_MotionState(lua_State *L)
{
  ...
  luaL_register(L, NULL, btMotionState_member_methods);
  // save meta-table in bt
  dub_register(L, "bt", "MotionState", "MotionState");
  // <mt>
  lua_pop(L, 1);
  return 0;
}
