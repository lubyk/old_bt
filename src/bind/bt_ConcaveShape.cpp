...
static int btConcaveShape__cast_(lua_State *L) {
  ...
  btConcaveShape *self = *((btConcaveShape **)dub_checksdata_n(L, 1, "bt.ConcaveShape"));
...
static int btConcaveShape__btConcaveShape(lua_State *L) {
  try {
    DubUserdata *userdata = ((DubUserdata*)dub_checksdata_d(L, 1, "bt.ConcaveShape"));
    if (userdata->gc) {
    ...
    lua_pushfstring(L, "~btConcaveShape: %s", e.what());
  } catch (...) {
    lua_pushfstring(L, "~btConcaveShape: Unknown exception");
  }
  return dub_error(L);
}
...
extern "C" int luaopen_bt_ConcaveShape(lua_State *L)
{
  ...
  luaL_register(L, NULL, btConcaveShape_member_methods);
  // save meta-table in bt
  dub_register(L, "bt", "ConcaveShape", "ConcaveShape");
  // <mt>
  lua_pop(L, 1);
  return 0;
}
