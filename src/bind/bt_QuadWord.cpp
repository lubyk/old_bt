...
static int btQuadWord__btQuadWord(lua_State *L) {
  try {
    DubUserdata *userdata = ((DubUserdata*)dub_checksdata_d(L, 1, "bt.QuadWord"));
    if (userdata->gc) {
    ...
    lua_pushfstring(L, "~btQuadWord: %s", e.what());
  } catch (...) {
    lua_pushfstring(L, "~btQuadWord: Unknown exception");
  }
  return dub_error(L);
}
...
static int btQuadWord_getX(lua_State *L) {
  try {
    btQuadWord *self = *((btQuadWord **)dub_checksdata(L, 1, "bt.QuadWord"));
 ...
 * src/vendor/bullet/src/LinearMath/btQuadWord.h:183
 */
static int btQuadWord_btQuadWord(lua_State *L) {
  try {
    ...
    if (top__ >= 4) {
      ...
      dub_pushudata(L, retval__, "bt.QuadWord", true);
      return 1;
    } else if (top__ >= 3) {
      btScalar _x = dub_checknumber(L, 1);
      btScalar _y = dub_checknumber(L, 2);
...
static int btQuadWord___tostring(lua_State *L) {
  btQuadWord *self = *((btQuadWord **)dub_checksdata_n(L, 1, "bt.QuadWord"));
...
extern "C" int luaopen_bt_QuadWord(lua_State *L)
{
  ...
  luaL_register(L, NULL, btQuadWord_member_methods);
  // save meta-table in bt
  dub_register(L, "bt", "QuadWord", "QuadWord");
  // <mt>
  lua_pop(L, 1);
  return 0;
}
