...
static int btDefaultMotionState__btDefaultMotionState(lua_State *L) {
  try {
    DubUserdata *userdata = ((DubUserdata*)dub_checksdata_d(L, 1, "bt.DefaultMotionState"));
    if (userdata->gc) {
    ...
    lua_pushfstring(L, "~btDefaultMotionState: %s", e.what());
  } catch (...) {
    lua_pushfstring(L, "~btDefaultMotionState: Unknown exception");
  }
  return dub_error(L);
}
...
static int btDefaultMotionState__set_(lua_State *L) {
  ...
  btDefaultMotionState *self = *((btDefaultMotionState **)dub_checksdata_n(L, 1, "bt.DefaultMotionState"));
  const char *key = luaL_checkstring(L, 2);
  int key_h = dub_hash(key, 4);
  switch(key_h) {
    case 3: {
      if (DUB_ASSERT_KEY(key, "m_graphicsWorldTrans")) break;
      self->m_graphicsWorldTrans = **((btTransform **)dub_checksdata_n(L, 3, "bt.Transform"));
      return 0;
    }
    case 1: {
      if (DUB_ASSERT_KEY(key, "m_centerOfMassOffset")) break;
      self->m_centerOfMassOffset = **((btTransform **)dub_checksdata_n(L, 3, "bt.Transform"));
      return 0;
    }
    case 2: {
      if (DUB_ASSERT_KEY(key, "m_startWorldTrans")) break;
      self->m_startWorldTrans = **((btTransform **)dub_checksdata_n(L, 3, "bt.Transform"));
      return 0;
    }
    case 0: {
      if (DUB_ASSERT_KEY(key, "m_userPointer")) break;
      dub_protect(L, 1, 3, "m_userPointer");
      self->m_userPointer = *((void **)dub_checksdata_n(L, 3, "void"));
      return 0;
    }
  }
  if (lua_istable(L, 1)) {
...
static int btDefaultMotionState__get_(lua_State *L) {
  ...
  btDefaultMotionState *self = *((btDefaultMotionState **)dub_checksdata_n(L, 1, "bt.DefaultMotionState", true));
  const char *key = luaL_checkstring(L, 2);
  // <self> "key" <mt>
  // rawget(mt, key)
  lua_pushvalue(L, 2);
  ...
  switch(key_h) {
    case 3: {
      if (DUB_ASSERT_KEY(key, "m_graphicsWorldTrans")) break;
      dub_pushudata(L, &self->m_graphicsWorldTrans, "bt.Transform", false);
      return 1;
    }
    case 1: {
      if (DUB_ASSERT_KEY(key, "m_centerOfMassOffset")) break;
      dub_pushudata(L, &self->m_centerOfMassOffset, "bt.Transform", false);
      return 1;
    }
    case 2: {
      if (DUB_ASSERT_KEY(key, "m_startWorldTrans")) break;
      dub_pushudata(L, &self->m_startWorldTrans, "bt.Transform", false);
      return 1;
    }
    case 0: {
      if (DUB_ASSERT_KEY(key, "m_userPointer")) break;
 ...
 * src/vendor/bullet/src/LinearMath/btDefaultMotionState.h:22
 */
static int btDefaultMotionState_btDefaultMotionState(lua_State *L) {
  try {
    int top__ = lua_gettop(L);
    if (top__ >= 2) {
      btTransform *startTrans = *((btTransform **)dub_checksdata(L, 1, "bt.Transform"));
      btTransform *centerOfMassOffset = *((btTransform **)dub_checksdata(L, 2, "bt.Transform"));
      btDefaultMotionState *retval__ = new btDefaultMotionState(*startTrans, *centerOfMassOffset);
      dub_pushudata(L, retval__, "bt.DefaultMotionState", true);
      return 1;
    } else if (top__ >= 1) {
      btTransform *startTrans = *((btTransform **)dub_checksdata(L, 1, "bt.Transform"));
 ...
 * src/vendor/bullet/src/LinearMath/btDefaultMotionState.h:27
 */
static int btDefaultMotionState_getWorldTransform(lua_State *L) {
  try {
    btDefaultMotionState *self = *((btDefaultMotionState **)dub_checksdata(L, 1, "bt.DefaultMotionState"));
    btTransform *centerOfMassWorldTrans = *((btTransform **)dub_checksdata(L, 2, "bt.Transform"));
 ...
 * src/vendor/bullet/src/LinearMath/btDefaultMotionState.h:34
 */
...
extern "C" int luaopen_bt_DefaultMotionState(lua_State *L)
{
  ...
  luaL_register(L, NULL, btDefaultMotionState_member_methods);
  // save meta-table in bt
  dub_register(L, "bt", "DefaultMotionState", "DefaultMotionState");
  // <mt>
  lua_pop(L, 1);
  return 0;
}
